...
/* 노선ID와 구간정보로 차량들의 위치정보를 조회한다.
 * 노선ID(busRouteId) 시작정류소 순번(startOrd) 종료정류소 순번(endOrd) 를 입력 받는다.  */
public class getBusPosByRouteStList extends Thread {
	Handler mhandler;
	int mwhat;
	int count;

	String addr = "http://ws.bus.go.kr/api/rest/buspos/";
	String function = "getBusPosByRouteSt";
	String servicekey;
	...
	getBusPosByRouteStList_Element Data;

	public getBusPosByRouteStList(Handler phandler, int pwhat) {
		mhandler = phandler;
		mwhat = pwhat;
	...
	public getBusPosByRouteStList(Handler phandler, int pwhat, String serviceKey, String parameter1, String parameter2, String parameter3) {
		mhandler = phandler;
		mwhat = pwhat;
	...
	public void run() {
    	Data = new getBusPosByRouteStList_Element();
	...
	public getBusPosByRouteStList_Element getElement() {
		return Data;
		...
		try {
			...
			while (parserEvent != XmlPullParser.END_DOCUMENT ) {
				...
				case XmlPullParser.TEXT:
					...
					case 7:
						Data.routeId.add( Integer.parseInt( parser.getText() ) );
						break;
